...
export async function setupWorld(provider: DojoProvider) {
  // System definitions for `jn_loot-game_system` contract
  function game_system() {
    ...
    const use_adventurer = async (props: {
      account: Account;
      game_id: number;
      ...
      try {
        return await provider.execute(
          props.account,
          {
            ...
            calldata: [props.game_id, props.adventurer_id],
          },
          "jn_loot"
        );
      } catch (error) {
    ...
    const skip_adventurer = async (props: {
      account: Account;
      game_id: number;
    }) => {
      try {
    ...
    const select_aventurer_cards = async (props: {
      account: Account;
      game_id: number;
      cards_index: number[];
    }) => {
    ...
    const skip_unpassed_obstacle = async (props: {
      account: Account;
      game_id: number;
    }) => {
      try {
    ...
    return {
      ...
      use_adventurer,
      skip_adventurer,
      select_aventurer_cards,
      skip_unpassed_obstacle,
      name,
    };
  }

  // System definitions for `jn_loot-poker_hand_system` contract
  ...
  // System definitions for `jn_loot-rage_system` contract
