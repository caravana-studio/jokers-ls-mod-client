...
export const getMultiPointEvents = (events: DojoEvent[]) => {
  return events
    .filter(
      (event) =>
        event.keys[0] === SPECIAL_MODIFIER_POINTS_EVENT ||
        event.keys[0] === SPECIAL_MODIFIER_MULTI_EVENT
    )
    .map((event) => {
      ...
      let points;
      let multi;
      if (event.keys[0] === SPECIAL_MODIFIER_POINTS_EVENT) {
        points = getNumberValueFromEvent(event, 3) ?? 0;
      }
      if (event.keys[0] === SPECIAL_MODIFIER_MULTI_EVENT) {
